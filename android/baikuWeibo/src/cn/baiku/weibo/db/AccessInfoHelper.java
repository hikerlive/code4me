...
import cn.baiku.weibo.bean.AccessInfo;
...
public class AccessInfoHelper {
	...
	public boolean update(AccessInfo accessInfo) {
		...
		String whereClause = AccessInfoColumn.USERID + "="
				+ accessInfo.getUserID();

		return newsDB.update(DBHelper.ACCESSLIB_TABLE, values, whereClause,
				null) > 0;
	}
	...
	public ArrayList<AccessInfo> getAccessInfos() {
		...
		Cursor cursor = newsDB.query(DBHelper.ACCESSLIB_TABLE,
				AccessInfoColumn.PROJECTION, null, null, null, null, null);
		...
		if (cursor.getCount() > 0) {
			for (cursor.moveToFirst(); !cursor.isAfterLast(); cursor
				...
				accessInfo.setUserID(cursor
						.getString(AccessInfoColumn.USERID_COLUMN));
				accessInfo.setAccessToken(cursor
						.getString(AccessInfoColumn.ACCESS_TOKEN_COLUMN));
				accessInfo.setAccessSecret(cursor
						.getString(AccessInfoColumn.ACCESS_SECRET_COLUMN));
				list.add(accessInfo);
			}
		}
	...
	public AccessInfo getAccessInfo(String userID) {
		...
		Cursor cursor = newsDB.query(DBHelper.ACCESSLIB_TABLE,
				AccessInfoColumn.PROJECTION, selection, null, null, null, null);
