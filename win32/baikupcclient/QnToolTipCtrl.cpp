...
IMPLEMENT_DYNAMIC(CQnToolTipCtrl, CToolTipCtrl)
CQnToolTipCtrl::CQnToolTipCtrl()
{
	m_arrowColor=RGB(31,152,214);
 	m_bkColor=RGB(255,255,255);
 	m_leftColor=RGB(208,234,244);
//	m_frameColor=RGB(96,119,140);
...
BEGIN_MESSAGE_MAP(CQnToolTipCtrl, CToolTipCtrl)
	ON_WM_PAINT()
	ON_WM_ERASEBKGND()
	ON_WM_NCPAINT()
END_MESSAGE_MAP()
...
// CQnToolTipCtrl message handlers


void CQnToolTipCtrl::PreSubclassWindow()
{
 //   ModifyStyle (WS_BORDER,0, 0);
	CWnd::PreSubclassWindow();
}

void CQnToolTipCtrl::OnPaint()
{
	POINT Pos;
	GetCursorPos(&Pos);
	CRect PRect;
	...
	GetClientRect(Rect);
	dc.FillRect(Rect,&CBrush(m_bkColor));
	R1=Rect;
	R1.right=R1.left+15;
...
BOOL CQnToolTipCtrl::OnEraseBkgnd(CDC* pDC)
{
	return CToolTipCtrl::OnEraseBkgnd(pDC);
}

void CQnToolTipCtrl::OnNcPaint()
{
}
